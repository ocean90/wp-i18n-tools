...
/**
 * Responsible for extracting functions calls from JS source files.
 */
class Function_Extractor_JS extends Function_Extractor {
	...
	public function find_function_calls() {
		...
		$tokens = j_token_get_all( $this->source );
		...
		foreach ( $tokens as $token ) {
			...
			if ( J_IDENTIFIER === $id && in_array( $text, $this->functions_to_extract ) && ! $in_func ) {
				$in_func = true;
				$paren_level = -1;
				$args = array();
				$func_name = $text;
			...
			if ( $this->comment_prefix && J_COMMENT === $id ) {
