...
( function( $, undef ){
	...
	ColorPicker = {
		...
		_create: function() {
			...
			self.wrap = el.parent();
			self.toggler = $( _before ).insertBefore( el ).css( { backgroundColor: self.initialValue } ).attr( 'title', wp.i18n.__( 'Select Color' ) ).attr( 'data-current', wp.i18n.__( 'Current Color' ) );
			self.pickerContainer = $( _after ).insertAfter( el );
			self.button = $( _button );

			if ( self.options.defaultColor ) {
				self.button.addClass( 'wp-picker-default' ).val( wp.i18n.__( 'Default' ) );
			} else {
				self.button.addClass( 'wp-picker-clear' ).val( wp.i18n.__( 'Clear' ) );
			}
