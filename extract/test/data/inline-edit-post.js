...
inlineEditPost = {
	...
	setBulk : function(){
		...
		if ( 'post' === type ) {
			...
			tax = 'post_tag';
			$('tr.inline-editor textarea[name="tax_input['+tax+']"]').suggest( ajaxurl + '?action=ajax-tag-search&tax=' + tax, { delay: 500, minchars: 2, multiple: true, multipleSep: wp.i18n._x( ',', 'tag delimiter' ).trim() } );
		}
	...
	edit : function(id) {
		...
		$('.tags_input', rowData).each(function(){
			...
			textarea.suggest( ajaxurl + '?action=ajax-tag-search&tax=' + taxname, { delay: 500, minchars: 2, multiple: true, multipleSep: wp.i18n._x( ',', 'tag delimiter' ).trim() } );
		});
		...
		$.post( ajaxurl, params,
			function(r) {
				...
				if (r) {
					...
					$errorSpan.html( wp.i18n.__( 'Error while saving the changes.' ) ).show();
					wp.a11y.speak( wp.i18n.__( 'Error while saving the changes.' ), 'polite' );
				}
			},
		'html');
