...
( function( $ ) {
	...
	tagBox = {
		clean : function(tags) {
			var comma = wp.i18n._x( ',', 'tag delimiter' );
			if ( ',' !== comma )
		...
		parseTags : function(el) {
			var id = el.id,
				...
				comma = wp.i18n._x( ',', 'tag delimiter' ),
		...
		quickClicks : function( el ) {
			...
			current_tags = thetags.val().split( wp.i18n._x( ',', 'tag delimiter' ) );
			tagchecklist.empty();
		...
		init : function() {
			...
			$('input.newtag', ajaxtag).keyup(function(e){
				...
				$(this).suggest(
					...
					{ delay: 500, minchars: 2, multiple: true, multipleSep: wp.i18n._x( ',', 'tag delimiter' ) }
				);
			});
