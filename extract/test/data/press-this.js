	...
	var PressThis = function() {
		...
		function submitPost( action ) {
			...
			$.ajax( {
				...
				renderError( wp.i18n.__( 'Connection lost or the server is busy. Please try again later.' ) );
			});
		}
		...
		function addImg( src, displaySrc, i ) {
			...
			$element.attr( 'data-wp-src', src ).css( 'background-image', 'url(' + displaySrc + ')' )
				...
				.find( 'span' ).text( wp.i18n.__( 'Suggested image #%d' ).replace( '%d', i + 1 ) );
		...
		function renderDetectedMedia() {
			...
			if ( data._embeds ) {
				$.each( data._embeds, function ( i, src ) {
					...
					$element.attr( 'data-wp-src', src ).find( 'span' ).text( wp.i18n.__( 'Suggested embed #%d' ).replace( '%d', i + 1 ) );
