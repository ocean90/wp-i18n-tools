...
var getCount, updateCount, updateCountText, updatePending, updateApproved,
	...
	updateHtmlTitle = function ( diff ) {
		...
		titleRegEx = titleRegEx || new RegExp( wp.i18n.__( 'Comments (%s)' ).replace( '%s', '\\([0-9' + thousandsSeparator + ']+\\)' ) + '?' );
		...
		if ( titleCount >= 1 ) {
			...
			if ( regExMatch ) {
				/* translators: %s: comments count */
				newTitle = document.title.replace( regExMatch[0], wp.i18n.__( 'Comments (%s)' ).replace( '%s', titleDiv.text() ) + ' ' );
			}
		} else {
			regExMatch = titleRegEx.exec( newTitle );
			if ( regExMatch ) {
				newTitle = newTitle.replace( regExMatch[0], wp.i18n.__( 'Comments' ) );
			}
		}
	...
	dimAfter = function( r, settings ) {
		...
		if ( c.is('.unapproved') ) {
			if ( settings.data.id == replyID )
				replyButton.text( wp.i18n.__( 'Approve and Reply' ) );
			...
			if ( settings.data.id == replyID )
				replyButton.text( wp.i18n.__( 'Reply' ) );
...
commentReply = {
	...
	toggle : function(el) {
		if ( 'none' !== $( el ).css( 'display' ) && ( $( '#replyrow' ).parent().is('#com-reply') || window.confirm( wp.i18n.__( "Are you sure you want to edit this comment?\nThe changes you made will be lost." ) ) ) ) {
