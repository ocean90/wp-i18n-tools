/* global ajaxurl, wpAjax, postboxes, pagenow, tinymce, alert, deleteUserSetting */
...
jQuery(document).ready( function($) {
	...
	$document.on( 'autosave-disable-buttons.edit-post', function() {
		...
		$( '.autosave-message' ).text( wp.i18n.__( 'Saving...' ) ); // No ellipsis.
	...
	if ( $('#submitdiv').length ) {
		...
		updateText = function() {
			...
			if ( $postVisibilitySelect.find('input:radio:checked').val() == 'private' ) {
				$('#publish').val( wp.i18n.__( 'Update' ) );
				if ( 0 === optPublish.length ) {
					postStatus.append('<option value="publish">' + wp.i18n.__( 'Privately Published' ) + '</option>');
				} else {
					optPublish.html( wp.i18n.__( 'Privately Published' ) );
				}
				...
				if ( $('#original_post_status').val() == 'future' || $('#original_post_status').val() == 'draft' ) {
					...
					optPublish.html( wp.i18n.__( 'Published' ) );
				}
			...
			if ( $('option:selected', postStatus).val() == 'private' || $('option:selected', postStatus).val() == 'publish' ) {
				...
				if ( $('option:selected', postStatus).val() == 'pending' ) {
					$('#save-post').show().val( wp.i18n.__( 'Save as Pending' ) );
				} else {
					$('#save-post').show().val( wp.i18n.__( 'Save Draft' ) );
				}
			}
			return true;
		};
		...
		$postVisibilitySelect.find('.save-post-visibility').click( function( event ) { // crazyhorse - multiple ok cancels
			...
			switch ( $postVisibilitySelect.find( 'input:radio:checked' ).val() ) {
				case 'password':
					visibility = wp.i18n.__( 'Password Protected' );
					break;
				case 'private':
					visibility = wp.i18n.__( 'Private' );
					break;
				default:
					visibility = $( '#sticky' ).prop( 'checked' ) ? wp.i18n.__( 'Public, Sticky' ) : wp.i18n.__( 'Public' );
					break;
			}
