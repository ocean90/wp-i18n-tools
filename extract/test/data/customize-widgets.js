	...
	api.Widgets.WidgetControl = api.Control.extend({
		...
		_setupReorderUI: function() {
			...
			$reorderNav.find( '.move-widget, .move-widget-down, .move-widget-up' ).each( function() {
				...
				if ( $( this ).is( '.move-widget' ) ) {
					...
					if ( isMoveUp ) {
						self.moveUp();
						wp.a11y.speak( wp.i18n.__( 'Widget moved up' ) );
					} else {
						self.moveDown();
						wp.a11y.speak( wp.i18n.__( 'Widget moved down' ) );
					}
		...
		_setupRemoveUI: function() {
			...
			replaceDeleteWithRemove = function() {
				$removeBtn.text( wp.i18n.__( 'Remove' ) ); // wp_widget_control() outputs the link as "Delete"
				$removeBtn.attr( 'title', wp.i18n.__( 'Trash widget by moving it to the inactive widgets sidebar.' ) );
			};
		...
		updateWidget: function( args ) {
			...
			jqxhr.done( function( r ) {
				...
				if ( r.success ) {
					...
					message = wp.i18n.__( 'An error has occurred. Please reload the page and try again.' );
		...
		ready: function () {
			...
			panel.deferred.embedded.done(function() {
				...
				noRenderedAreasNotice.append( $( '<em></em>', {
					text: wp.i18n.__( 'There are no widget areas currently rendered in the preview. Navigate in the preview to a template that makes use of a widget area in order to access its widgets here.' )
				} ) );
	...
	api.Widgets.SidebarControl = api.Control.extend({
		...
		toggleReordering: function( showOrHide ) {
			...
			if ( showOrHide ) {
				...
				wp.a11y.speak( wp.i18n.__( 'Reorder mode enabled' ) );
				...
				wp.a11y.speak( wp.i18n.__( 'Reorder mode closed' ) );
