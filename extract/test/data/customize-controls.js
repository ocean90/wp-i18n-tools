/* global _wpCustomizeHeader, _wpCustomizeBackground, MediaElementPlayer */
(function( exports, $ ){
	...
	api.HeaderControl = api.Control.extend({
		...
		openMedia: function(event) {
			...
			this.frame = wp.media({
				button: {
					text: wp.i18n.__( 'Select and Crop' ),
					close: false
				},
				states: [
					new wp.media.controller.Library({
						title:     wp.i18n.__( 'Choose Image' ),
						library:   wp.media.query({ type: 'image' }),
						multiple:  false,
						date:      false,
						priority:  20,
	...
	api.PreviewFrame = api.Messenger.extend({
		...
		run: function( deferred ) {
			...
			this.request.done( function( response ) {
				...
				self.iframe = $( '<iframe />', { 'title': wp.i18n.__( 'Site Preview' ) } ).appendTo( self.container );
		...
		login: function( deferred ) {
			...
			$.get( api.settings.url.ajax, {
				...
				iframe = $( '<iframe />', { 'src': self.previewUrl(), 'title': wp.i18n.__( 'Site Preview' ) } ).hide();
	...
	api.Previewer = api.Messenger.extend({
		...
		login: function() {
			...
			iframe = $( '<iframe />', { 'src': api.settings.url.login, 'title': wp.i18n.__( 'Session expired' ) } ).appendTo( this.container );
		...
		cheatin: function() {
			$( document.body ).empty().addClass( 'cheatin' ).append(
				'<h1>' + wp.i18n.__( 'Cheatin&#8217; uh?' ) + '</h1>' +
				'<p>' + wp.i18n.__( 'Sorry, you are not allowed to customize this site.' ) + '</p>'
			);
		},
	...
	$( function() {
		...
		(function() {
			...
			state.bind( 'change', function() {
				if ( ! activated() ) {
					saveBtn.val( wp.i18n.__( 'Save &amp; Activate' ) ).prop( 'disabled', false );
					closeBtn.find( '.screen-reader-text' ).text( wp.i18n.__( 'Cancel' ) );

				} else if ( saved() ) {
					saveBtn.val( wp.i18n.__( 'Saved' ) ).prop( 'disabled', true );
					closeBtn.find( '.screen-reader-text' ).text( wp.i18n.__( 'Close' ) );

				} else {
					saveBtn.val( wp.i18n.__( 'Save &amp; Publish' ) ).prop( 'disabled', false );
					closeBtn.find( '.screen-reader-text' ).text( wp.i18n.__( 'Cancel' ) );
				}
			});
		...
		api.state( 'paneVisible' ).bind( function( paneVisible ) {
			...
			if ( ! paneVisible ) {
				$( '.collapse-sidebar' ).attr({ 'aria-expanded': 'false', 'aria-label': wp.i18n.__( 'Expand Sidebar' ) });
			} else {
				$( '.collapse-sidebar' ).attr({ 'aria-expanded': 'true', 'aria-label': wp.i18n.__( 'Expand Sidebar' ) });
			}
		});
		...
		if ( title.length ) {
			api( 'blogname', function( setting ) {
				var updateTitle = function() {
					title.text( $.trim( setting() ) || wp.i18n.__( '(Untitled)' ) );
				};
