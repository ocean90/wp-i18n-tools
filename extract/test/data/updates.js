 ...
 * @param {bool}    settings.isNetworkAdmin             Whether we're in the network admin or not.
...
(function( $, wp, settings ) {
	...
	/**
	 * Whether we're in the network admin or not.
	 *
	 * @since 4.7.0
	 *
	 * @type {bool}
	 */
	wp.updates.isNetworkAdmin = settings.isNetworkAdmin;
	...
	wp.updates.updatePlugin = function( args ) {
		...
		if ( 'plugins' === pagenow || 'plugins-network' === pagenow ) {
			...
			message    = wp.i18n.__( 'Updating %s...' ).replace( '%s', $updateRow.find( '.plugin-title strong' ).text() );
		} else if ( 'plugin-install' === pagenow || 'plugin-install-network' === pagenow ) {
			$card    = $( '.plugin-card-' + args.slug );
			$message = $card.find( '.update-now' ).addClass( 'updating-message' );
			/* translators: %s: Plugin name and version */
			message  = wp.i18n.__( 'Updating %s...' ).replace( '%s', $message.data( 'name' ) );

			// Remove previous error messages, if any.
			$card.removeClass( 'plugin-card-update-failed' ).find( '.notice.notice-error' ).remove();
		}

		if ( $message.html() !== wp.i18n.__( 'Updating...' ) ) {
			$message.data( 'originaltext', $message.html() );
		}

		$message
			.attr( 'aria-label', message )
			.text( wp.i18n.__( 'Updating...' ) );
	...
	wp.updates.updatePluginSuccess = function( response ) {
		...
		$updateMessage
			/* translators: %s: Plugin name and version */
			.attr( 'aria-label', wp.i18n.__( '%s updated!' ).replace( '%s', response.pluginName ) )
			.text( wp.i18n.__( 'Updated!' ) );

		/* translators: JavaScript accessible string */
		wp.a11y.speak( wp.i18n.__( 'Update completed successfully.' ), 'polite' );
	...
	wp.updates.updatePluginError = function( response ) {
		...
		errorMessage = wp.i18n.__( 'Update Failed: %s' ).replace( '%s', response.errorMessage );
		...
		if ( 'plugins' === pagenow || 'plugins-network' === pagenow ) {
			...
			if ( response.pluginName ) {
				$message.find( 'p' )
					/* translators: %s: Plugin name and version */
					.attr( 'aria-label', wp.i18n.__( '%s update failed' ).replace( '%s', response.pluginName ) );
			} else {
			...
			$card.find( '.update-now' )
				.text( wp.i18n.__( 'Update Failed!' ) ).removeClass( 'updating-message' );
	...
	wp.updates.installPlugin = function( args ) {
		...
		if ( $message.html() !== wp.i18n.__( 'Installing...' ) ) {
			$message.data( 'originaltext', $message.html() );
		}
		...
		$message
			...
			/* translators: %s: Plugin name and version */
			.attr( 'aria-label', wp.i18n._x( 'Installing %s...', 'plugin' ).replace( '%s', $message.data( 'name' ) ) )
			.text( wp.i18n.__( 'Installing...' ) );

		wp.a11y.speak( wp.i18n.__( 'Installing... please wait.' ), 'polite' );
	...
	wp.updates.installPluginSuccess = function( response ) {
		var $message            = $( '.plugin-card-' + response.slug ).find( '.install-now' ),
		    activatePlugin      = wp.i18n.__( 'Activate' ),
		    /* translators: %s: Plugin name */
		    activatePluginLabel = wp.i18n._x( 'Activate %s', 'plugin' );

		$message
			.removeClass( 'updating-message' )
			.addClass( 'updated-message installed button-disabled' )
			/* translators: %s: Plugin name and version */
			.attr( 'aria-label', wp.i18n._x( '%s installed!', 'plugin' ).replace( '%s', response.pluginName ) )
			.text( wp.i18n.__( 'Installed!' ) );

		wp.a11y.speak( wp.i18n.__( 'Installation completed successfully.' ), 'polite' );

		$document.trigger( 'wp-plugin-install-success', response );

		if ( response.activateUrl ) {
			setTimeout( function() {
				if ( wp.updates.isNetworkAdmin ) {
					activatePlugin = wp.i18n.__( 'Network Activate' );
					/* translators: %s: Plugin name */
					activatePluginLabel = wp.i18n._x( 'Network Activate %s', 'plugin' );
				}
				...
				$message.removeClass( 'install-now installed button-disabled updated-message' ).addClass( 'activate-now button-primary' )
					...
					.attr( 'aria-label', activatePluginLabel.replace( '%s', response.pluginName ) )
					.text( activatePlugin );
			}, 1000 );
		}
	};
	...
	wp.updates.installPluginError = function( response ) {
		...
		errorMessage = wp.i18n.__( 'Installation failed: %s' ).replace( '%s', response.errorMessage );
		...
		$button
			...
			/* translators: %s: Plugin name and version */
			.attr( 'aria-label', wp.i18n._x( '%s installation failed', 'plugin' ).replace( '%s', $button.data( 'name' ) ) )
			.text( wp.i18n.__( 'Install Failed!' ) );
	...
	wp.updates.installImporterSuccess = function( response ) {
		wp.updates.addAdminNotice( {
			...
			message:   wp.i18n.__( 'Importer installed successfully. <a href="%s">Run importer</a>' ).replace( '%s', response.activateUrl + '&from=import' )
		} );
		...
		$( '[data-slug="' + response.slug + '"]' )
			...
			.attr({
				...
				'aria-label': wp.i18n.__( 'Run %s' ).replace( '%s', response.pluginName )
			})
			.text( wp.i18n.__( 'Run Importer' ) );
	...
	wp.updates.installImporterError = function( response ) {
		/* translators: %s: Error string for a failed installation */
		var errorMessage = wp.i18n.__( 'Installation failed: %s' ).replace( '%s', response.errorMessage ),
		...
		$installLink
			.removeClass( 'updating-message' )
			.text( wp.i18n.__( 'Install Now' ) )
			/* translators: %s: Plugin name */
			.attr( 'aria-label', wp.i18n.__( 'Install %s now' ).replace( '%s', pluginName ) );
	...
	wp.updates.deletePlugin = function( args ) {
		...
		if ( $link.html() !== wp.i18n.__( 'Deleting...' ) ) {
			$link
				.data( 'originaltext', $link.html() )
				.text( wp.i18n.__( 'Deleting...' ) );
		}

		wp.a11y.speak( wp.i18n.__( 'Deleting...' ), 'polite' );
		...
		$( '[data-plugin="' + response.plugin + '"]' ).css( { backgroundColor: '#faafaa' } ).fadeOut( 350, function() {
			...
			if ( plugins.all.length ) {
				...
				if ( ! $form.find( 'tr.no-items' ).length ) {
					$form.find( '#the-list' ).append( '<tr class="no-items"><td class="colspanchange" colspan="' + columnCount + '">' + wp.i18n.__( 'You do not appear to have any plugins available at this time.' ) + '</td></tr>' );
				}
			}
		} );

		wp.a11y.speak( wp.i18n.__( 'Deleted!' ), 'polite' );
	...
	wp.updates.updateTheme = function( args ) {
		...
		if ( $notice.html() !== wp.i18n.__( 'Updating...' ) ) {
			$notice.data( 'originaltext', $notice.html() );
		}

		/* translators: JavaScript accessible string */
		wp.a11y.speak( __( 'Updating... please wait.' ), 'polite' ); // No ellipsis.
		$notice.text( wp.i18n.__( 'Updating...' ) ); // No ellipsis.
	...
	wp.updates.installTheme = function( args ) {
		...
		$message
			.text( wp.i18n__( 'Installing...' ) )
			/* translators: %s: Theme name and version */
			.attr( 'aria-label', wp.i18n._x( 'Installing %s...', 'theme' ).replace( '%s', $message.data( 'name' ) ) );
		wp.a11y.speak( wp.i18n.__( 'Installing... please wait.' ), 'polite' );
	...
	wp.updates.installThemeSuccess = function( response ) {
		var $card              = $( '.wp-full-overlay-header, [data-slug=' + response.slug + ']' ),
		    activateTheme      = wp.i18n.__( 'Activate' ),
		    activateThemeLabel = wp.i18n._x( 'Activate %s', 'theme' ),
		    $message;
		...
		$message = $card.find( '.button-primary' )
			...
			/* translators: %s: Theme name and version */
			.attr( 'aria-label', wp.i18n._x( '%s installed!', 'theme' ).replace( '%s', response.themeName ) )
			.text( wp.i18n.__( 'Installed!' ) );
		...
		setTimeout( function() {
			...
			if ( response.activateUrl ) {
				if ( wp.updates.isNetworkAdmin ) {
					activateTheme = wp.i18n.__( 'Network Enable' );
					/* translators: %s: Theme name */
					activateThemeLabel = wp.i18n._x( 'Network Activate %s', 'theme' );
				}
				...
				$message
					...
					.addClass( 'activate' )
					.attr( 'aria-label', activateThemeLabel.replace( '%s', response.themeName ) )
					.text( activateTheme );
			}
	...
	wp.updates.installThemeError = function( response ) {
		...
		$button
			.removeClass( 'updating-message' )
			/* translators: %s: Theme name and version */
			.attr( 'aria-label', wp.i18n._x( '%s installation failed', 'theme' ).replace( '%s', $button.data( 'name' ) ) )
			.text( wp.i18n.__( 'Install Failed!' ) );
	...
	wp.updates.deleteTheme = function( args ) {
		...
		if ( $button && $button.html() !== wp.i18n.__( 'Deleting...' ) ) {
			$button
	...
	wp.updates.deleteThemeError = function( response ) {
		var $themeRow    = $( 'tr.inactive[data-slug="' + response.slug + '"]' ),
			...
			errorMessage = wp.i18n.__( 'Deletion failed: %s' ).replace( '%s', response.errorMessage ),
			$message     = wp.updates.adminNotice( {
				className: 'update-message notice-error notice-alt',
				message:   errorMessage
			} );
	...
	wp.updates.isValidResponse = function( response, action ) {
		var error = wp.i18n.__( 'An unknown error occurred' ),
		    errorMessage;
		...
		if ( _.isString( response ) && '-1' === response ) {
			error = wp.i18n.__( 'An error has occurred. Please reload the page and try again.' );
		} else if ( _.isString( response ) ) {
			error = response;
		} else if ( 'undefined' !== typeof response.readyState && 0 === response.readyState ) {
			error = wp.i18n.__( 'Connection lost or the server is busy. Please try again later.' );
		...
		switch ( action ) {
			case 'update':
				/* translators: %s: Error string for a failed update */
				errorMessage = wp.i18n.__( 'Update Failed: %s' );
				break;

			case 'install':
				/* translators: %s: Error string for a failed installation */
				errorMessage = wp.i18n.__( 'Installation failed: %s' );
				break;

			case 'delete':
				/* translators: %s: Error string for a failed deletion */
				errorMessage = wp.i18n.__( 'Deletion failed: %s' );
				break;
		}
	...
	wp.updates.beforeunload = function() {
		if ( wp.updates.ajaxLocked ) {
			return wp.i18n.__( 'Updates may not complete if you navigate away from this page.' );
		}
	};
	...
	$( function() {
		...
		$document.on( 'credential-modal-cancel', function( event, job ) {
			...
			if ( $message && $message.hasClass( 'updating-message' ) ) {
				...
				if ( 'plugin-install' === pagenow || 'plugin-install-network' === pagenow ) {
					if ( 'update-plugin' === job.action ) {
						/* translators: %s: Plugin name and version */
						$message.attr( 'aria-label', wp.i18n.__( 'Update %s now' ).replace( '%s', $message.data( 'name' ) ) );
					} else if ( 'install-plugin' === job.action ) {
						/* translators: %s: Plugin name */
						$message.attr( 'aria-label', wp.i18n.__( 'Install %s now' ).replace( '%s', $message.data( 'name' ) ) );
					}
				}
			}

			/* translators: JavaScript accessible string */
			wp.a11y.speak( wp.i18n.__( 'Update canceled.' ), 'polite' );
		} );
		...
		$bulkActionForm.on( 'click', '[data-plugin] a.delete', function( event ) {
			...
			/* translators: %s: Plugin name */
			if ( ! window.confirm( wp.i18n.__( 'Are you sure you want to delete %s and its data?' ).replace( '%s', $pluginRow.find( '.plugin-title strong' ).text() ) ) ) {
				return;
			}
		...
		$document.on( 'click', '.themes-php.network-admin a.delete', function( event ) {
			...
			/* translators: %s: Theme name */
			if ( ! window.confirm( wp.i18n.__( 'Are you sure you want to delete %s?' ).replace( '%s', $themeRow.find( '.theme-title strong' ).text() ) ) ) {
				return;
			}
		...
		$bulkActionForm.on( 'click', '[type="submit"]', function( event ) {
			...
			if ( ! itemsSelected.length ) {
				...
				return wp.updates.addAdminNotice( {
					...
					className: 'notice-error is-dismissible',
					message:   wp.i18n.__( 'Please select at least one item to perform this action on.' )
				} );
			}
			...
			switch ( bulkAction ) {
				...
				case 'delete-selected':
					bulkDeleteMsg = wp.i18n.__( 'Caution: These themes may be active on other sites in the network. Are you sure you want to proceed?' );

					if ( 'plugin' === type ) {
						bulkDeleteMsg = wp.i18n.__( 'Are you sure you want to delete the selected plugins and their data?' );
					}

					if ( ! window.confirm( bulkDeleteMsg ) ) {
						event.preventDefault();
						return;
					}
		...
		$pluginInstallSearch.on( 'keyup input', _.debounce( function( event, eventtype ) {
			...
			if ( ! $searchTab.length ) {
				$searchTab = $( '<li class="plugin-install-search" />' )
					.append( $( '<a />', {
						'class': 'current',
						'href': searchLocation,
						'text': wp.i18n.__( 'Search Results' )
					} ) );
			...
			wp.updates.searchRequest = wp.ajax.post( 'search-install-plugins', data ).done( function( response ) {
				...
				if ( 0 === response.count ) {
					wp.a11y.speak( wp.i18n.__( 'No plugins found. Try a different search.' ) );
				} else {
					wp.a11y.speak( wp.i18n._n( '%d plugin found', '%d plugins found', response.count ) );
				}
			} );
		}, 500 ) );
		...
		$pluginSearch.on( 'keyup input', _.debounce( function( event ) {
			...
			wp.updates.searchRequest = wp.ajax.post( 'search-plugins', data ).done( function( response ) {
				...
				var $subTitle    = $( '<span />' ).addClass( 'subtitle' ).html( wp.i18n.__( 'Search results for &#8220;%s&#8221;' ).replace( '%s', _.escape( data.s ) ) ),
